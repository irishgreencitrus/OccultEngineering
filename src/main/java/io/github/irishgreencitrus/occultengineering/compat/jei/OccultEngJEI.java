...
public class OccultEngJEI implements IModPlugin {
    ...
    private void loadCategories() {
        ...
        CreateRecipeCategory<?> enspirit = builder(SpiritFireRecipe.class)
                .addTypedRecipes(OccultismRecipes.SPIRIT_FIRE_TYPE)
                .catalystStack(() -> {
                    var fan = AllBlocks.ENCASED_FAN.asStack();
                    fan.applyComponents(DataComponentMap.builder()
                            .set(DataComponents.ITEM_NAME, OccultEngineeringLang.translateDirect("recipe.fan_enspirit.fan")).build());
                    return fan;
                })
                .doubleItemIcon(AllItems.PROPELLER.get(), OccultismBlocks.SPIRIT_CAMPFIRE)
    ...
    private class CategoryBuilder<T extends Recipe<? extends RecipeInput>> {
        ...
        public <I extends RecipeInput, R extends Recipe<I>> CategoryBuilder<T> addTypedRecipes(Supplier<RecipeType<R>> recipeType) {
            return addRecipeListConsumer(recipes -> OccultEngJEI.<T>consumeTypedRecipes(recipe -> {
        ...
        public CategoryBuilder<T> addTypedRecipes(Supplier<RecipeType<T>> recipeType, Function<RecipeHolder<?>, RecipeHolder<T>> converter) {
            return addRecipeListConsumer(recipes -> OccultEngJEI.<T>consumeTypedRecipes(recipe -> recipes.add(converter.apply(recipe)), recipeType.get()));
        }
    ...
    public static boolean doInputsMatch(Recipe<?> recipe1, Recipe<?> recipe2) {
        ...
        return recipe2.getIngredients().get(0)
